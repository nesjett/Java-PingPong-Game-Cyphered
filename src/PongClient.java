...
public class PongClient extends JFrame implements KeyListener, Runnable, WindowListener{
    ...
    // - Constructor - //
	public PongClient(String clientname, String portAdd, String ipAdd, Test parent){
		this.parent = parent;
        	 ...
        	 if(clientSoc.isConnected()){
            	System.out.println("TEST");
            	 ...
            	 // - Generate new secret key for the match (AES) - //
	       		 ...
	       		 SecurityData sDataC = new SecurityData();
	       		 sDataC.setPublic(PSecurity.getPublic());
        		 ObjectOutputStream sendObj = new ObjectOutputStream(clientSoc.getOutputStream());
              	 sendObj.writeObject(sDataC);
              	 sendObj = null;
	       		 
	       		 
	       		 // - Get server public key + secret key encripted with RSA and signed from client - //
        		 ObjectInputStream getObj = new ObjectInputStream(clientSoc.getInputStream());
				 SecurityData sDataS = (SecurityData) getObj.readObject();
				 getObj = null;
				 System.out.println("Client security data received: " + sDataS.toString());
				 System.out.println("Signed and Encripted secret key: " + sDataS.getSecret());

        		 
				 // - Unsign secret key - //
				 String unsignedSecret = PSecurity.unsignObject(sDataS.getSecret());
				 System.out.println("Secret key encrypted: " + unsignedSecret);
				 
				 // - NULL if signature failed (we are under attack?) - //
				 if(unsignedSecret == null){
					 clientSoc.close();
				 }
				 
				 // - Decript secret key - //
				 byte[] secret = PSecurity.decryptWithPrivateKey(unsignedSecret, PSecurity.getPrivate());
				 System.out.println("Secret key: " + secret.toString());
        		 PSecurity.setSecret(secret);
            	 ...
            	 while(true){
            		...
            		// - Creating Streams - //
            		 //ObjectOutputStream sendObj = new ObjectOutputStream(clientSoc.getOutputStream());
            		 PSecurity.secretEncryptAndSend(playerC, clientSoc.getOutputStream());
        			 ...
        			 //ObjectInputStream getObj = new ObjectInputStream(clientSoc.getInputStream());
        			 playerS = (PlayerServer) PSecurity.AESDecryptStream(clientSoc.getInputStream());
